...
switchRule
    : switchLabel guard? '->' (expression ';' | block | throwStatement)
    ;

switchBlockStatementGroup
    : switchLabel guard? ':' (switchLabel guard? ':')* blockStatements
    ;

switchLabel
    : 'case' caseConstant (',' caseConstant)*
    | 'case' unannType identifier
    | 'case' recordPattern
    | 'default'
    ;
...
recordPattern
    : unannType '(' componentPatternList? ')'
    ;
